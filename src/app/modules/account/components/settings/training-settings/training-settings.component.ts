...
@Component({
  ...
  styleUrl: './training-settings.component.scss'
})
export class TrainingSettingsComponent implements OnInit, OnDestroy {
  ...
  saveSettings() {
    ...
    newTenant.setTenantTrainingSettingsData(result);
    const registrationSubscr = this.apiService
      ...
      .subscribe((tenant) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Successful',
          detail: 'Training Settings Updated Successfullly',
          life: 2000,
      });
      });
  ...
  initForm() {
    this.form = new FormGroup({
      enableContentSurveysForAllNewTrainingCampaigns: new FormControl<boolean>(false, [Validators.required]),
      enableDirectLinksInTrainingNotifications: new FormControl<boolean>(false, [Validators.required]),
